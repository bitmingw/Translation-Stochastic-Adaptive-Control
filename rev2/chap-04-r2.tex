...
现在我们分析连续时间链和离散时间链之间的关系。设$\{x(t)\}$为一个时变链，其速率矩阵为$Q$。假设
\begin{equation*}
Q_0 := \sup_i (-Q_ii) = \sup_i \sum_{j \ne i} Q_ij < \infty
\end{equation*}
则称此马尔可夫链是可均匀化的。（注意，当状态空间是有限的时，$Q_0 < \infty$。）设定值$q \ge Q_0$，概率分布$x(t)$满足
\begin{equation*}
...
因此
\begin{align}
p(t) &= p(0) exp \ -tq [I - R] \nonumber \\ % The equation needs to be checked
&= p(0) \sum_{k=0}^{\infty} [exp - tq] \frac{(tq)^k}{k!} R^k \label{eq:4.8.1}
\end{align}

因为$R$是一个随机矩阵，考虑一个离散时间链$\{x_k\}$以$R$作为状态转移概率，且初始概率分布为$p(0)$，则$x_k$的概率分布为
\begin{equation*}
p_k = p(0) R^k
\end{equation*}
与式\eqref{eq:4.8.1}相比，可得
\begin{equation}
p(t) = \sum_{k=0}^{\infty} [exp - tq] \frac{(qt)^k}{k!} \label{eq:4.8.2} % Needs to be checked
\end{equation}
令$\{K(t)\}$为速率为$q$的泊松过程，且与链$\{x_k\}$无关，则有
\begin{equation}
Prob\{K(t) = k\} = [exp - tq] \frac{(qt)^k}{k!}, \quad k = 0,1,2,... \label{eq:4.8.3}
\end{equation}
若进一步，从$\{K(t)\}$和$\{x(t)\}$得出连续时间过程$\{\hat{x}(t)\}$
\begin{equation*}
\hat{x}(t) := x_{K(t)}
\end{equation*}
也就是说，在$t$时刻$\hat{x}$的值是离散时间链$x_k$在时间$k = K(t)$的值。
% ==== End of Page 54 ====
另外，设$T_0 := 0$，对任意$k > 0$，设$T_k$是第一次$K(t) = k$的时间，则
\begin{equation}
\hat{x}(t) = x_k, \quad (T_k \le t < T_{k+1}) \label{eq:4.8.4}
\end{equation}
这里需要注意，$T_{k+1} - T_k, k = 0,1,...$是服从概率密度$p(T_{k+1} - T_k = t) = q \ exp-qt$分布的随机变量。这是由于泊松过程中跳变发生的时间是服从指数分布的。从贝叶斯定理出发，
\begin{equation}
P\{\hat{x}(t) = i\} = \sum_{k=0}^{\infty} P\{x_k = i\} \ P\{K(t) = k\} \label{eq:4.8.5}
\end{equation}
从\eqref{eq:4.8.2}，\eqref{eq:4.8.3}和\eqref{eq:4.8.5}出发，我们可以得出
\begin{equation*}
P\{\hat{x}(t) = i\} = P\{x(t) = i\}
\end{equation*}
故$x(t)$和$\hat{x}(t)$具有相同的概率分布。

下面我们通过一个控制案例：串联的两个排队站来解释均匀化的步骤。假设客户以速率为$\lambda$的泊松过程到达站$1$，在$t$时刻站的服务速率可以为$u(t) \in [0,a]$间的任意数。在站$1$完成服务时，一个客户加入第二个队列，其服务速度为恒定值$\mu$。设$x_i(t)$为$t$时刻第$i$个服务站积累的客户数量，包括正在服务的客户。指定$x(t) = (x_1(t),x_2(t)) \in \{0,1,2,...\} \times \{0,1,2,...\}$代表$t$时刻的状态。

令$p(t) = \{p(x(t)) = (i,j), 0 \le i,j < \infty\}$为$x(t)$的概率分布。则它的柯尔莫哥洛夫方程
\begin{align*}
...
\end{align*}
显然每个速率的绝对值都小于$q := \lambda + \mu + a$，因此我们得到了一个离散时间链，其非零转移概率由下面的公式给出：
\begin{align*}
...
我们现在将连续时间和离散时间马尔可夫链的代价关联起来。设$c(i)$是$t$时刻瞬间的代价（当$x(t) = i$时）,$c := (c(1),c(2),...)^T$。在无限时间内$\{x(t)\}$的代价的期望，考虑到衰减率$\alpha > 0$，为：
\begin{align*}
E \ & \int_0^{\infty} [exp -\alpha t] c(x(t)) dt \\ % Need to be checked
&= \int_0^{\infty} [exp - \alpha t] p(t) c dt \\
&= \sum_{k=0}^{\infty} \{\int_0^{\infty} [exp -(\alpha+q)t] \frac{(qt)^k}{k!}\} p_k c, \quad by \eqref{eq:4.8.2} \\
&= (\alpha + q)^{-1} \sum_{k=0}^{\infty} \beta^k p_k c
\end{align*}
% ==== End of Page 56 ====
其中$\beta := q (\alpha+q)^{-1}$。因此忽略常数项$(\alpha+q)^{-1}$，它与离散时间链$\{x_k\}$，在衰减率$\beta$的情况下的代价相同。
...
1. 可控马尔可夫链模型的介绍参见 Howard (1971)。有限马尔可夫链在 Kemeny and Snell (1960) 和 Feller (1957) 中有简要介绍。我们可以在其中找到引理\eqref{Lem:4.5.4}，\eqref{Lem:4.5.7}和\eqref{Lem:4.5.11}的证明。连续时间马尔可夫链在 Doob (1953) 中有简要介绍。

2. 根据引理\eqref{Lem:4.5.11}，无周期性和不可约的有限链是各态历经性的。无线链没有这样简明的性质。相关内容可以见 Kemeny, Snell and Knapp (1976) 和 Revuz (1975)。

3. 如果无限连续时间链$\{x(t)\}$是可均匀化的，即$\sup_i \sum_{j \ne i} Q_ij = \infty$，则$x(t)$可能在一段有限的时间内无限次地变更状态，这意味着它可能没有对应的离散时间链。一个连续链$\{x(t)\}$可以通过时间离散化来近似，只需要令$x_k = x(\Delta k), \Delta > 0$。参见 Kushner (1977) 和 Dijk (1984)。

4. 泊松过程可用于描述更加一般的状态跳转过程，包括连续时间马尔可夫链，参见 Bremaud (1981) 和 Snyder (1975)。

% ==== End of Page 57 ====

5. 排队模型在 Kleinrock (1975), Bremaud (1981) 和 Gross and Harris (1974) 中都有描述。更一般的排队网络见 Kelly (1979)。最近关于排队系统在控制问题中的应用参见 Stidham (1985)。 第8节的例子出自 Rosberg, Varaiya and Walrand (1981)。
