...
此外，若将练习\eqref{Exe:3.1.17}中的系统变为时变系统，一阶和二阶矩恰好相同的情况仍然成立。
...
\begin{Prf}
因为$A$是稳定的，所以存在数$K < \infty$和$0 < \alpha < 1$使$A^j$的每个系数的模都小于$K \ \alpha^j$。因此公式\eqref{eq:3.3.3}中的第一项变成了0，且整个级数可求和。因此
\begin{equation*}
	\lim_{k \to \infty} \Sigma_k = \Sigma_{\infty} := \sum_{j=0}^{\infty} A^j GQG^T (A^j)^T.
\end{equation*}
接下来，由于$\Sigma_k$收敛至$\Sigma_{\infty}$，我们让\eqref{eq:3.3.1}中的$k \to \infty$，归纳出$\Sigma_{\infty}$满足公式\eqref{eq:3.3.5}。因此，只需要证明公式\eqref{eq:3.3.5}有唯一解。假设$\Sigma_{\infty}^1$和$\Sigma_{\infty}^2$是\eqref{eq:3.3.5}的两个解。那么他们的差值$\Delta := \Sigma_{\infty}^1 - \Sigma_{\infty}^2$满足
\begin{equation*}
	\Delta \ = \ A \Delta A^T.
\end{equation*}
递归带入此式，可得
