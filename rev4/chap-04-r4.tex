...
如果\eqref{eq:4.6.3}中的塞萨罗极限存在（在有限状态下总是存在），那么有
\begin{equation*}
...
\end{align}
前两个关系式说明，当$x(t) = 1$时，$x(t+dt) = 2$的概率为$[q_1 - q_2] dt$，它的补集$x(t+dt) = 1$的概率是$1 - [q_1 - q_2] dt$。另外，这些转移概率并不取决于先前的状态和控制量，即当$s < t$时转移概率与$x(s), u(s)$无关，这是马尔可夫的性质。相似的表述对后两个关系式也适用。
...
\end{equation}
式\eqref{eq:4.7.2}称为柯尔莫哥洛夫前向方程，\eqref{eq:4.7.3}是\eqref{eq:4.3.2}的对应。若$g(t,i) \equiv g(i)$是时不变的，则$Q^g$是静态的，且有
\begin{equation*}
	P^g(s,t) = exp([(t-s) Q^g]), \quad p(t) = p(0) \ exp(tQ^g).
\end{equation*}

\begin{Exe} \label{Exe:4.7.4}
令$Q$为一个速率矩阵，其非对角线元素均为非负，且每一行元素之和为$0$。设$x(t) := (x_1(t),...,x_n(t))$满足微分方程$\dot{x}(t) = x(t) \ Q$。试证明对所有$t$，均有$\sum_{i=1}^n x_i(t)$为常数。因此只要$x(0)$是一个概率分布，则对任意$t$，$x(t)$各组成部分之和为$1$。 \\
...
在离散马尔可夫链的情况下，如果对所有$t \ge s$和$p(s) = \pi$可以得到$p(t) = \pi$，也就是说$\dot{p}(t) \equiv 0$，则概率分布$\pi$是时不变的。从\eqref{eq:4.7.2}我们可知
\begin{equation}
...
\textnormal{[}提示：通过将指数矩阵展开成多项式级数，证明$\pi \ exp(tQ) = \pi$。\textnormal{]}
\end{Exe}
...
\end{equation*}
总是存在的。它是时不变的概率分布，满足$p(\infty) Q = 0$。（我们没有必要取引理\eqref{Lem:4.5.4}中的塞萨罗极限。）就像离散时间的情形一样，若马尔可夫链是可约的，$p(\infty)$的值取决于初始概率分布$p(0)$。另外，对无限状态的马尔可夫链，该极限并不始终存在。

下面的两个例子讲解了马尔可夫链在排队系统中的应用。

\begin{Exe} \label{eq:4.7.10}
有这样一个队列，顾客以泊松过程的速率$\lambda$进入队列。队列由一台服务器负责，一次可以处理一个顾客，且每次完成服务的时间是一个以$\mu^{-1}$为均值的呈指数分布的随机变量。设$x(t)$为在$t$时刻队列中的顾客数量。
% ==== End of Page 52 ====
