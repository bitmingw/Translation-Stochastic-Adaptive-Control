...
在本章将介绍有限可控马尔可夫模型下随机系统的一些基本结论。在完全可观和反馈法则只取决于当前状态时，状态过程被称为马尔可夫链。本章将复习马尔可夫链的渐进性质，并简要介绍无限状态马尔可夫链。最后，本章将提出一个在某些情形下把连续时间马尔可夫链转化为离散时间马尔可夫链的方法。我们将通过一个排序系统讲解此方法。

\section{一个例子}

迄今为止我们假设状态变量的取值范围均为$R^n$，但在很多情况下，最好让状态只能取一个有限集合的数值。下面我们引入一个例子。

有这样一台机器，其$k$时刻的状态由$x_k$来描述，根据机器是否正常运转，$x_k$的值可以为$1$或$2$。假定现在不允许从外界对机器施加控制，机器处于自治状态。若在$k$时刻机器正常运行，则$x_k = 1$，此时它有一定的概率$q > 0$会在下一时刻崩溃，也就是$x_{k+1} = 2$；同时，它有$1 - q$的概率维持正常运行的状态，即$x_{k+1} = 1$。进一步假设$q$与先前的状态$x_{k-1},...,x_0$无关，并且崩溃的机器会一直保持崩溃的状态（当$x_k = 2$时$x_{k+1} = 2$的概率为1）。此时称$\{x_k, k \ge 0\}$是一个马尔可夫链，它的转移概率由矩阵$P = P\{P_{ij}\}$描述：
\begin{equation}
	P = 
	\begin{bmatrix}
		1-q & q \\
...
下面我们引入两个控制动作。设$u_k^1$是$k$时刻机器工作的强度。当机器停止工作，轻度工作和重度工作时$u_k^1$的值分别取0，1和2。机器的工作强度越高，它崩溃的可能性就越大。设$u_k^2$是维护机器所需的投入，它仅可以取值0或1，越大的值代表越多的维护。维护可以降低机器崩溃的概率，并使崩溃的机器恢复正常状态。
...
前面的例子可以一般化成任意有限可控马尔可夫链。它的状态$x_k$可取值$\{1,...,I\}$。它的控制可以从一个有限或无限的集合$U$中取值。转移概率由一个以$U$中的值为自变量，大小为$I \times I$的矩阵描述
\begin{equation}
...
\begin{equation*}
	P_k^g ... P_{k+m-1}^g.
\end{equation*}
矩阵中第$ij$个元素是指当$k$时刻系统状态为$i$的条件下$k+m$时刻它的状态为$j$的概率。即
\begin{equation*}
...
马尔可夫策略$g$决定了状态过程$\{x_k\}$的概率分布和控制过程$\{u_k = g_k(x_k)\}$。不同的策略会导致不同的概率分布。在最优控制中，我们希望找到最好的策略，这需要通过比较多种策略才能得出。为此需要定义代价函数，它是一系列和状态与控制相关的实值函数
\begin{equation*}
...
\end{equation}
此处的$E^g$代表由$g$决定的$\{x_k\}$和$\{u_k\}$的概率分布下的均值。
...
\textnormal{[}$\pi$被称为马尔可夫链$\{x_k\}$的稳态或不变概率分布。\textnormal{]}
\end{Lem}
...
\textnormal{[}随机时间$\tau$叫做停止时间，这种问题称为停止时间问题。\textnormal{]}
\end{Exe}
...
由马尔可夫的性质可知，
\begin{equation*}
	p_{k+m} = p_k \ P_k^g ... P_{k+m-1}^g, \quad \text{and } \quad p_k = p_0 \ P_0^g ... P_{k-1}^g
\end{equation*}
...
\begin{equation}
	\pi \ P = \pi, \quad \textnormal{and } \quad \sum_{i=0}^{\infty} \pi(i) = 1
\end{equation}
当且仅当$\epsilon > \frac{1}{2}$ \\
\textnormal{[}提示：先证明$\pi \ P = \pi$的一个正数解必须与$\eta = (\eta_1,\eta_2,...)$成正比。其中
\begin{equation*}
	\eta(1) = 1, \quad \textnormal{and } \quad \eta(i) = \frac{(1 - \epsilon)^{i-2}}{\epsilon^{i-1}}, \quad i > 1
\end{equation*}
...
\textnormal{[}提示：在微分方程的左右两边同时乘以列向量$(1,...,1)^T$。\textnormal{]}
\end{Exe}
...
\textnormal{[}提示：通过将矩阵的幂展开成多项式级数，证明$\pi \ exp(tQ) = \pi$。\textnormal{]}
\end{Exe}
...
\end{equation*}
总是存在的。并且它是时不变的概率分布，满足$p(\infty) Q = 0$。（我们没有必要取引理\eqref{Lem:4.5.4}中的塞萨罗极限）另外，就像离散时间的情形一样，若马尔可夫链是可约的，$p(\infty)$的值取决于初始概率分布$p(0)$。另外，对无限状态的马尔可夫链，极限并不是始终存在。
...
\begin{equation}
	p(t) = \sum_{k=0}^{\infty} [exp(-tq)] \frac{(qt)^k}{k!} \label{eq:4.8.2}
\end{equation}
...
\begin{align*}
	...
	&= \sum_{k=0}^{\infty} \{\int_0^{\infty} [exp(-(\alpha+q)t)] \frac{(qt)^k}{k!}\} p_k c, \quad \textnormal{由} \eqref{eq:4.8.2} \\
...
2. 根据引理\eqref{Lem:4.5.11}，无周期性和不可约的有限链是各态历经性的。无线链没有这样简明的性质。相关内容可以见 Kemeny, Snell and Knapp (1976)\cite{Kemeny76DMC} 和 Revuz (1975)\cite{Revuz75MC}。

3. 如果无限连续时间链$\{x(t)\}$是可均匀化的，即$\sup_i \sum_{j \ne i} Q_{ij} = \infty$，则$x(t)$可能在一段有限的时间内无限次地变更状态，这意味着它可能没有对应的离散时间链。一个连续链$\{x(t)\}$可以通过时间离散化来近似，只需要令$x_k = x(\Delta k), \Delta > 0$。参见 Kushner (1977)\cite{Kushner77PM} 和 Dijk (1984)\cite{Dijk84CMP}。
