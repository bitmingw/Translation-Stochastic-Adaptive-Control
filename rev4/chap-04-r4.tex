...
上面的排队系统称为$M/M/1$队列。第一个$M$代表到达队列的过程是一个泊松过程，这是一个时间间隔无记忆的马尔可夫过程。第二个$M$代表服务时间以指数形式分布，且其结果是无记忆的。最后的$1$代表只有一个服务商。

下个练习是一个有关电话交换的模型。

\begin{Exe}
一个电话交换系统共有$n$条电话线。电话发起的过程可用一个速率为$\lambda$的泊松过程描述。当电话发起时，如果有一条空闲的电话线，则它将会被占用。如果所有的电话线都被占用，电话将被转移或者阻断。当一条线被占用时，其保持忙碌的时间是一个以$\mu^{-1}$为均值的指数分布的随机变量。令$x(t)$为在$t$时刻被占用的电话线数量，此时$x(t)$是一个连续时间马尔可夫过程，其速率矩阵$Q$由下面的几个公式描述
\begin{align*}
	Q_{i,i-1} :=& \ i_{\mu}, \quad i = 1,...,n, \\
	Q_{i,i+1} :=& \ \lambda, \quad i = 0,...,n-1, \\
	Q_00 :=& - \lambda, \quad Q_nn := - n \mu, \\
	Q_ii :=& -(\lambda + i \mu), \quad i = 1,...,n-1.
\end{align*}
试证明存在一个时不变概率分布$\pi$满足
\begin{equation*}
	\pi_i = \frac{\rho^i / i!}{\sum_{j=0}^n \rho^j / j!}, \quad i \ge 0.
\end{equation*}
而$\pi_n$是所有线路都繁忙的稳态概率，它也是电话丢失或阻断的概率，被称为厄兰丢失公式。
% ==== End of Page 53 ====
...
下面我们通过一个控制案例--串联的两个排队站--来解释均匀化的步骤。假设客户以速率为$\lambda$的泊松过程到达站$1$，在$t$时刻站的服务速率可以设定为$u(t) \in [0,a]$间的任意数。在站$1$完成服务时，一个客户加入第二个队列，其服务速率为恒定值$\mu$。设$x_i(t)$为$t$时刻第$i$个服务站积累的客户数量，包括正在服务的客户。令$x(t) = (x_1(t),x_2(t)) \in \{0,1,2,...\} \times \{0,1,2,...\}$代表$t$时刻的状态。
