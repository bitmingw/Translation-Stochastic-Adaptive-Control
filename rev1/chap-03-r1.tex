...
对于n维随机变量$z$，
\begin{equation}
z \sim N(\overline{z},\Sigma) \quad or \quad p(z) \sim N(\overline{z},\Sigma) \label{eq:3.1.1}
\end{equation}
...
\begin{equation}
x_0 \sim N(\overline{x_0},\Sigma_0), \quad w_k \sim N(0,Q), \quad v_k \sim N(0,R). \label{eq:3.1.4}
\end{equation}
...
\begin{equation}
x_{k+m} = A^m \ x_k + \sum_{j=0}^{m-1} A^{m-1-j} B \ u_{k+j} + \sum_{j=0}^{m-1} A^{m-1-j} G \ w_{k+j}. \label{eq:3.1.7}
\end{equation}
因为随机变量$\{w_{k+j},j \ge 0\}$服从高斯分布，且与$x_k$，$u_k$，$u_{k+j},j \ge 1$相互独立，故从公式\eqref{eq:3.1.7}可知
\begin{equation}
p(x_{k+m} \vert x_k,u_k,...,u_{k+m-1}) \sim N(A^m x_k + \sum_{j=0}^{m-1} A^{m-1-j} B \ u_{k+j}, \Sigma_{k+m \vert k}) \label{eq:3.1.8}
\end{equation}
...
且独立变量服从高斯分布
\begin{equation*}
x_0 \sim N(\overline{x}_0, \Sigma_0), \quad w_k \sim N(0, Q_k), \quad v_k \sim N(0, R_k).
\end{equation*}
请证明$m$步转移概率也服从高斯分布
\begin{align*}
