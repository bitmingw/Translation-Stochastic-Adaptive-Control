...
在本章将介绍有限可控马尔可夫模型下随机系统的一些基本结论。在完全可观以及反馈法则只取决于当前状态时，状态过程被称为马尔可夫链。本章将复习马尔可夫链的渐进性质，并简要介绍无限状态马尔可夫链。最后，本章将提出一个在某些情形下把连续时间马尔可夫链转化为离散时间马尔可夫链的方法。我们将通过一个排序系统的例子进行讲解。
...
考虑一个状态空间为$\{0,1,...\}$的马尔可夫链。令$Q_{i,i+1} := \lambda > 0$且$Q_{ii} := - \lambda$。因此马尔可夫链以$\lambda$的速率向下一个更大的整数移动。设初始的概率分布$p(0) = (1,0,0,0,...)$，即马尔可夫链从状态$0$开始。证明对所有$i \ge 0$，有$p_i(t) = exp(- \lambda t)(\lambda t)^i / i!$。 \\
