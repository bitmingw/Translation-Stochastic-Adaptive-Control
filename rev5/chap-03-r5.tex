...
某些时候，即使$A$不是稳定的，$\Sigma_k$仍旧会收敛。一个简单的例子是当$\Sigma_0 = 0$，$Q = 0$时，$\Sigma_k \equiv 0$而与$A$无关。另外，$\Sigma_{\infty}$也不必是严格正定的，例如当$A = 0$且$GQG^T < n$的时候，$\Sigma_k$也是收敛的。上面的两个例子说明，如果输入扰动$\{w_k\}$影响了状态向量的每个组成部分，那么$A$的稳定性是$\Sigma_k$收敛的必要条件，且协方差的极限$\Sigma_{\infty}$是正定的。如果合适地定义了影响每个组成部分的记号，上面的断言是有说服力的。
\end{Rem}
...
\begin{equation*}
	x_{k+1} = A_k \ x_k + w_k
\end{equation*}
给出。如果$x_0,w_0,w_1,...$是相互独立的且服从高斯分布，那么这个过程就是高斯-马尔可夫过程。我们把这个这个命题的逆命题作为练习。

假设$\{x_k\}$是一个高斯-马尔可夫过程，$E \ x_k \equiv 0$，指定$\Sigma_k := cov(x_k)$，$\Sigma_{k+1,k} := cov(x_{k+1},x_k)$。
...
\textnormal{[}提示：\eqref{eq:3.4.3}的第一个等式可以由马尔可夫的性质\eqref{eq:3.4.1}得到。它的第二个等式和余下的推导，可通过计算$x_{k+1} - \Sigma_{k+1,k} \Sigma_k^{-1} x_k$和$x_k$之间的协方差证明他们之间没有相关关系。由于两个耦合的高斯随机变量是非相关的，所以他们是相互独立的，也就是说$E \ \{\hat{x}_{k+1} - \Sigma_{k+1,k} \Sigma_{k}^{-1} x_k \ \vert \ x_k \} = 0$，因此\eqref{eq:3.4.3}的第二项也得到了验证。\textnormal{]}
\end{Exe}
